...
1. **[传统]** 证明：如果 $\psi(x)$ 满足归一化条件，那么 $c\psi(x)$($c$ 为常数)也满足归一化条件当且仅当 $|c| = 1$。这说明波函数的**相位**不影响概率分布。
2. **[传统]** 考虑波函数 $\psi(x) = A e^{-\alpha|x|}$，求归一化常数 $A$，并计算粒子出现在 $|x| < 1/\alpha$ 范围内的概率。
3. **[思考]** 波函数的叠加原理如何解释双缝干涉实验？为什么"概率的叠加"与"概率幅的叠加"给出不同的结果？
4. **[计算]** 运行上面的电子云可视化代码，探索不同量子数 $(n, l, m)$ 组合对应的电子云形状。为什么 $l=0$(s 轨道)总是球对称的？
