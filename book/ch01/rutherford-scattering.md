...
1. 能量为 7.7 MeV 的α粒子轰击金箔（$Z = 79$），求正面碰撞时的最近距离。与金原子核的半径（约 7 fm）比较，判断卢瑟福公式在此能量下是否仍然适用。

2. 在卢瑟福散射实验中，如果将入射粒子从α粒子换成质子（相同动能），散射截面会如何变化？如果入射能量加倍呢？

3. 证明：在卢瑟福散射中，散射角大于 $\theta_0$ 的粒子所占的比例为 $\cot^2(\theta_0/2) \cdot (r_0/2b_{\max})^2$，其中 $b_{\max}$ 是实验中有效的最大瞄准距离。

4. 思考题：卢瑟福散射公式是基于经典力学推导的。当我们考虑量子效应时，这个公式需要修正吗？在什么条件下量子效应变得重要？

5. 编程题：修改本节的蒙特卡洛代码，加入原子核的有限大小效应。假设原子核是均匀带电球，半径为 $R = 7$ fm，模拟高能α粒子（如 20 MeV）的散射，观察散射角分布与点状原子核情况的偏差。
